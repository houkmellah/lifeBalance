...
import { TextInput, PasswordInput, Button, Paper, Title, Text, Container, Group, Notification, Anchor } from '@mantine/core';
...
const Auth = () => {
  ...
  const form = useForm({
    ...
    validate: {
      name: (value) => (mode === 'register' && value.length < 2 ? 'Name must have at least 2 characters' : null),
      email: (value) => (/^\S+@\S+$/.test(value) ? null : 'Invalid email'),
      password: (value) => (mode !== 'forgotPassword' && value.length < 6 ? 'Password must have at least 6 characters' : null),
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      if (mode === 'login') {
        ...
        setNotification({ type: 'success', message: 'Password reset instructions sent to your email.' });
        setMode('login');
      }
    } catch (error) {
  ...
  return (
    <Container size={420} my={40}>
      <Title align="center" sx={(theme) => ({ fontFamily: `Greycliff CF, ${theme.fontFamily}`, fontWeight: 900 })}>
        {mode === 'login' ? 'Login' : mode === 'register' ? 'Sign Up' : 'Forgot Password'}
      </Title>
      {notification && (
        <Notification
      ...
      <Group position="center" mt="md">
        {mode === 'login' && (
          <>
            ...
            <Button variant="subtle" size="sm" onClick={() => setMode('register')}>
              Sign Up
            </Button>
          </>
        )}
